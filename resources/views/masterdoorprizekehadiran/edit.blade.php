<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl leading-tight">
            {{ __('Edit Master Doorprize Kehadiran') }}
        </h2>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    <form action="{{ route('masterdoorprizekehadiran.update', $masterDoorprize) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-6 border-t pt-4">
                            ...
                            <div id="lokasi-container">
                                @php
                                    $existingLokasi = $masterDoorprize->lokasi->where('status', 1);
                                @endphp

                                @if($existingLokasi->count() > 0)
                                    @foreach($existingLokasi as $index => $lokasi)
                                        <div class="lokasi-row flex gap-4 mb-3 items-end" id="lokasi-row-{{ $index }}">
                                            <div class="flex-1">
                                                <label class="block text-gray-700 text-sm font-bold mb-2">Lokasi Event</label>
                                                <select name="lokasi[{{ $index }}][lokasi_event]" 
                                                        class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('lokasi.' . $index . '.lokasi_event') border-red-500 @enderror">
                                                    <option value="">Pilih Lokasi Event</option>
                                                    @foreach($lokasiEvents as $lokasiEvent)
                                                        <option value="{{ $lokasiEvent->nama_lokasi }}" 
                                                                {{ $lokasi->lokasi_event == $lokasiEvent->nama_lokasi ? 'selected' : '' }}>
                                                            {{ $lokasiEvent->nama_lokasi }}
                                                            @if($lokasiEvent->tanggal)
                                                                ({{ \Carbon\Carbon::parse($lokasiEvent->tanggal)->format('d/m/Y') }})
                                                            @endif
                                                        </option>
                                                    @endforeach
                                                </select>
                                                @error('lokasi.' . $index . '.lokasi_event')
                                                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                                                @enderror
                                            </div>
                                            <div class="flex-1">
                                                <label class="block text-gray-700 text-sm font-bold mb-2">Jumlah Doorprize</label>
                                                <input type="number" name="lokasi[{{ $index }}][jumlah_doorprize]" 
                                                       value="{{ old('lokasi.' . $index . '.jumlah_doorprize', $lokasi->jumlah_doorprize) }}"
                                                       class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('lokasi.' . $index . '.jumlah_doorprize') border-red-500 @enderror"
                                                       placeholder="Jumlah" min="0">
                                                @error('lokasi.' . $index . '.jumlah_doorprize')
                                                    <p class="text-red-500 text-xs italic mt-1">{{ $message }}</p>
                                                @enderror
                                            </div>
                                    ...
                                    @endforeach
                                @else
                                    <!-- Jika tidak ada lokasi, tampilkan 1 row kosong -->
    ...
    <script>
        let lokasiIndex = {{ $masterDoorprize->lokasi->where('status', 1)->count() > 0 ? $masterDoorprize->lokasi->where('status', 1)->count() : 1 }};
