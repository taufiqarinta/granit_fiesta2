...
if ($action === 'check_incoming') {
    ...
    if (empty($number)) {
        echo json_encode(['status' => false, 'error' => 'number wajib diisi']);
        exit;
    }

    $result = callWaApi('/check-incoming/' . rawurlencode($number), 'GET', null, 20);
    ...
    if (is_array($result['json'])) {
        ...
        echo json_encode([
            'status' => false,
            'error' => 'Response check-incoming tidak valid',
            'debug' => WA_PROXY_DEBUG ? [
                'url' => $result['url'] ?? null,
                'raw' => $result['raw'] ?? null
            ] : null
