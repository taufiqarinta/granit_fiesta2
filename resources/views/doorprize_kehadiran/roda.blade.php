...
function tembakConfetti() {
    confetti({ particleCount: 500, spread: 90, origin: { x: 1, y: 0.9 } });
    confetti({ particleCount: 500, spread: 90, origin: { x: 0, y: 0.9 } });
    confetti({ particleCount: 500, spread: 90, origin: { x: 1, y: 0.1 } });
    confetti({ particleCount: 500, spread: 90, origin: { x: 0, y: 0.1 } });
}
</script>
<script>
const lokasi = "{{ $lokasi }}";
...
function drawWheel() {
    ...
    peserta.forEach((p, i) => {
        ...
        ctx.textAlign = "right";
        ctx.fillText(truncateNama(p.nama_toko), radius - 20, 5);
        ctx.restore();
    });
}
...
async function spin() {
    ...
    let data;
    try {
        const res = await fetch(`/doorprize-kehadiran/${lokasi}/${doorprizeId}/start-roda`, {
            method: 'POST',
            headers: {
    ...
    setTimeout(() => {
        document.getElementById("winner").innerHTML =
            '<span class="nama-toko">' + pemenang.nama_toko + '</span>' +
            '<span class="nama-pic">' + pemenang.nama_pic + '</span>';
